...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
    ...
    // Get Penyewa ID
    $sql_get_transaksi_penyewa_id = "SELECT transaksi_penyewa_id from tb_transaksi WHERE transaksi_id = '$transaksi_id'";
    $data_transaksi_penyewa_id = mysqli_fetch_assoc(mysqli_query($conn, $sql_get_transaksi_penyewa_id));
    $transaksi_penyewa_id = $data_transaksi_penyewa_id['transaksi_penyewa_id'];

    // Get Buku ID
    $sql_get_transaksi_buku_id = "SELECT transaksi_buku_id from tb_transaksi WHERE transaksi_id = '$transaksi_id'";
    $data_transaksi_buku_id = mysqli_fetch_assoc(mysqli_query($conn, $sql_get_transaksi_buku_id));
    $transaksi_buku_id = $data_transaksi_buku_id['transaksi_buku_id'];

    // Get Paket ID
    $sql_get_transaksi_paket_id = "SELECT transaksi_paket_id from tb_transaksi WHERE transaksi_id = '$transaksi_id'";
    $data_transaksi_paket_id = mysqli_fetch_assoc(mysqli_query($conn, $sql_get_transaksi_paket_id));
    $transaksi_paket_id = $data_transaksi_paket_id['transaksi_paket_id'];
    ...
    $sql_insert_kode_akses = "INSERT INTO tb_akses (akses_id, akses_kode, akses_penyewa_id, akses_buku_id, akses_transaksi_id, akses_tanggal_awal, akses_tanggal_akhir) VALUES ('$akses_id', '$akses_kode', '$transaksi_penyewa_id', '$transaksi_buku_id', '$transaksi_id', '$starting_date', '$end_date')";
